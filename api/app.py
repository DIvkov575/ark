...
def set_model(config: Dict[str, Any]):
    ...
    if model_name in model_dict:
        ...
        raise KeyError(
            "Model '{}' not found in model dictionary: {}".format(
...
def _parse_multipart(response):
    ...
    for part in parts:
        ...
        file_storage = werkzeug.datastructures.FileStorage(
            stream=dicom_file, filename=filename
        )
...
def _predict_wrapper(app, _parse_function):
    ...
    try:
        ...
        response["data"] = _predict_dicom_files(
            app, model, dicom_files, payload, return_attentions=return_attentions
        )
...
def set_routes(app):  # noqa: C901
    ...
    def dicom():
        ...
        response, response_code, dicom_files = _predict_wrapper(
            app, _parse_form_request
        )
    ...
    def handle_store(study_instance_uid=None):
        ...
        if study_instance_uid is not None:
            app.logger.debug(
                f"Request received at /dicom-web/studies/{study_instance_uid}"
            )
        ...
        stow_response = {
            ...
            "00081199": {"vr": "SQ", "Value": success_studies},
            # Studies that failed to process
            "00081198": {"vr": "SQ", "Value": failed_studies},
            "data": response["data"],
        }
    ...
    def get_scores():
        ...
        try:
            ...
            scores_file_path = os.environ.get(
                ARK_SAVE_SCORES_PATH_KEY, DEFAULT_SAVE_PATH
            )
...
def safe_path(base_path, user_input) -> str:
    ...
    if (
        os.path.commonpath([base_path, os.path.join(base_path, normalized_path)])
        != base_path
    ):
