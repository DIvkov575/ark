...
class APIUtilsTestCase(unittest.TestCase):
    def setUp(self):
        self.app = build_app()
        self.data = {
            "data": json.dumps({"test": 12345}),
            "dicom": [(io.BytesIO(b"12345"), "test.png")],
        }

    def test_validate_post_request(self):
        with self.app.test_request_context(
            "/dicom/files", data=self.data, method="POST"
        ):
    ...
    def test_validate_post_request_max_size(self):
        with self.app.test_request_context(
            ...
            max_size = 0
            expected_str = "Request data too large: {} > {}".format(
                request.content_length, max_size
            )
